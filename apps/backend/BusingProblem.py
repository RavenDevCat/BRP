        ...
        try:
            ...
            for candidate in payload.get("results") or []:
                ...
                if not is_plausible_geocode_result(
                    country, city, lat, lng, formatted_address, requested_address=address
                ):
    ...
    for index, item in enumerate(input_records):
        ...
        if point is None:
            try:
                ...
                if index == 0:
                    raise RuntimeError("The school address could not be resolved precisely. No passenger stop was substituted as the school.") from exc
                continue
        point = dict(point)
