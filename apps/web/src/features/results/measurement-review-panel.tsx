...
import {
  ...
  type JobRecord, type MeasurementReviewMode, type MeasurementReviewRecord,
} from "@/lib/api";
import { reviewActions, reviewBudget, reviewHasNativeResult, reviewIsActive } from "@/lib/measurement-review-state";
...
export function MeasurementReviewWorkspace({ job, mode, children }: {
  job: JobRecord;
  ...
  const enabled = ["succeeded", "failed"].includes(job.status) && Boolean(job.result);
  const user = useQuery({ queryKey: ["me"], queryFn: getCurrentUser, staleTime: 60_000 });
  const listKey = ["measurement-reviews", job.job_id];
  const risks = useQuery({ queryKey: ["measurement-risk", job.job_id], queryFn: () => getMeasurementRisk(job.job_id), enabled, staleTime: Infinity });
  const list = useQuery({ queryKey: listKey, queryFn: () => listMeasurementReviews(job.job_id), enabled,
    refetchInterval: query => query.state.data?.reviews.some(reviewIsActive) ? 5000 : false });
  const detail = useQuery({ queryKey: [...listKey, selectedId], queryFn: () => getMeasurementReview(job.job_id, selectedId),
  ...
  const start = useMutation({
    mutationFn: (limit: number) => {
      ...
      return createMeasurementReview(job.job_id, { mode, provider_call_limit: limit,
  ...
  const action = useMutation({
    mutationFn: ({ row, command }: { row: MeasurementReviewRecord; command: "pause" | "resume" | "cancel" }) =>
      controlMeasurementReview(job.job_id, row.review_id, command),
    onSuccess: refresh,
  });
  ...
  const selected = detail.data?.review_id === selectedId && detail.data.source_job_id === job.job_id ? detail.data : undefined;
    ...
    <section className="min-w-0 border-y border-border bg-surface px-4 py-4" aria-label={t("Historical measurement review")}>
      ...
      {createOpen && admin ? <form className="mt-4 space-y-3 border-t border-border pt-4" onSubmit={event => { event.preventDefault(); if (canStart && confirmed && limit !== null && !busy) start.mutate(limit); }}>
        <div className="flex flex-wrap items-end gap-4">
          <div className="text-sm"><span className="text-muted-foreground">{t("Scope")}: </span>{t(mode === "full_audit" ? "All saved Audit plans" : "Complete student time analysis")} · {formatNumber(capability?.scope_summary?.route_count)} {t("routes")}</div>
      ...
      {selected ? <div className="mt-4 space-y-3 border-t border-border pt-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {reviewHasNativeResult(selected, mode) ? <a href={getMeasurementReviewExportUrl(job.job_id, selected.review_id)} className={buttonClassName("secondary")}><Download className="h-4 w-4" />{t("Export correction Excel")}</a> : null}
        </div>
        ...
        {selected.request.mode === "full_audit" ? <p className="text-xs text-muted-foreground">{t("Saved stop assignments and order are unchanged. The solver and minimum-fleet proof were not rerun.")}</p> : null}
...
function ReviewComparison({ record }: { record: MeasurementReviewRecord }) {
  ...
  return <details className="text-xs">
    ...
    <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left"><thead className="border-b border-border text-muted-foreground"><tr>
      ...
      <td className="p-2">{t(row.route_key.startsWith("time_constrained:") ? "Strict Plan" : row.route_key.startsWith("exception_preserving:") ? "Protected Plan" : "Current Plan")} · {row.route_id}</td>
