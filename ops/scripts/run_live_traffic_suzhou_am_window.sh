...
WINDOW_START="${BRP_LIVE_TRAFFIC_SUZHOU_AM_WINDOW_START:-07:00}"
WINDOW_END="${BRP_LIVE_TRAFFIC_SUZHOU_AM_WINDOW_END:-07:35}"
INTERVAL_SECONDS="${BRP_LIVE_TRAFFIC_SUZHOU_AM_WINDOW_INTERVAL_SECONDS:-300}"
...
while true; do
  ...
  "$ROOT_DIR/ops/scripts/run_live_traffic_suzhou_sampler.sh" am_peak --sample-due-routes-only --skip-empty
  now_epoch="$(date +%s)"
