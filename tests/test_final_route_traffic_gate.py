...
def test_amap_failure_forces_a_new_route_at_the_same_exact_vehicle_count(monkeypatch):
    monkeypatch.setattr(planner_core, "infer_traffic_location", lambda _records: ("CHINA", "Shanghai"))
    monkeypatch.setattr(planner_core, "FINAL_ROUTE_TRAFFIC_VERIFICATION_ENABLED", True)
    monkeypatch.setattr(planner_core, "FINAL_ROUTE_TRAFFIC_REPLAN_ENABLED", True)
    ...
    provider_calls: list[tuple[tuple[float, float], ...]] = []

    def fake_amap_route_stats(_planner, request_points, _cache, state):
        provider_calls.append(tuple(request_points))
        state["api_calls"] = int(state.get("api_calls", 0)) + 1
        return {
            "duration_s": 9000 if request_points[1][0] == 31.2 else 1200,
            "distance_m": 1234,
            "source": "fake_amap",
        }
    ...
    class FakePlanner:
        ...
        NODE_TIME_UPPER_BOUNDS = {}
        MIN_SOLVER_VEHICLE_COUNT = 0
        MAX_ROUTE_DURATION_SECONDS = 120 * 60
        OSRM_BASE_URL = ""
        INPUT_STOPS = [{"address": "Shanghai", "passenger_count": 1}]

        def __init__(self):
            self.solve_limits: list[float] = []
            self.solve_vehicle_counts: list[int] = []
            self._BRP_ACTIVE_CONFIG = planner_core.PlannerConfig(
                service_direction="To School",
                to_school_arrival_time="08:00",
            )
        ...
        def solve_routes(self, _points, _solve_time, _solve_distance):
            self.solve_limits.append(float(self.MAX_ROUTE_DURATION_SECONDS))
            self.solve_vehicle_counts.append(int(self.MIN_SOLVER_VEHICLE_COUNT or 0))
            original_route = self.MAX_ROUTE_DURATION_SECONDS >= 3000
            return [
                {
                    "route_id": "Bus 1",
                    "nodes": [0, 1, 2] if original_route else [0, 2, 1],
                    "time_s": 3000 if original_route else 2400,
                    "raw_osrm_time_s": 3000 if original_route else 2400,
                    "stop_service_time_s": 0,
                }
            ]
    ...
    result = planner_core._compute_scenario_without_render(
        planner,
        points,
        "exact-count-feedback",
        reduced_vehicle_limit=1,
        forced_vehicle_count=1,
    )

    assert result["traffic_gate"]["status"] == "passed"
    assert planner.solve_vehicle_counts == [1, 1]
    assert planner.solve_limits == [120 * 60, 2999]
    assert len(provider_calls) == 2
    assert provider_calls[0] != provider_calls[1]
    assert result["traffic_replan_attempts"][0]["exact_vehicle_count"] == 1
    assert result["traffic_replan_attempts"][0]["failed_route_ids"] == ["Bus 1"]
    assert result["traffic_replan_attempts"][0]["to_route_duration_minutes"] == 2999 / 60
